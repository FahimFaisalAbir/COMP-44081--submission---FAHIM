# -*- coding: utf-8 -*-
"""
Created on Thu Jun 12 20:06:32 2025
...
#         plt.show()
    ...
    def _prophet_seasonal_holiday_model(self, train, test, country='CA'):
        ...
        # Initialize Prophet with holiday effects
        
        #Additive: Seasonal effects have constant magnitude. sing additive seasonality can lead to underfitting if your seasonal
        #effect scales up/down with changing levels of data (i.e., multiplicative behavior).

        #Multiplicative: Seasonal effects scale proportionally with trend.
        ...
        # Trian/ fit Prophet model
        model.fit(train_df)

        # Forecast test period length using prophet 
        ...
        # prophet pred columns is yhat so fetch yhat as forecast
...
#         )
    ...
    def _xgboost_model(self, train, test):
        ...
        df['Redemption_lag7'] = df['Redemption Count'].shift(7)
        # Sales lag leads to overfitting someto
        #df['Sales_lag1'] = df['Sales Count'].shift(1)
        #'Sales_lag1'
        # Drop rows with NA as some intial lags might be 0 
        df.dropna(inplace=True)
        
#         features = ['Sales_lag1','Redemption_lag1','Redemption_lag7','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear', 
#                     'seasonal_base', 'lag7', 'lag14', 'lag28','lag30','lag56','lag_year']

        features = ['Redemption_lag1','Redemption_lag7','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear', 
                    'seasonal_base', 'lag7', 'lag14', 'lag28','lag30','lag56','lag_year']
...
#         print("best params",grid_search.best_params_)
        #best params {'learning_rate': 0.1, 'max_depth': 5, 'n_estimators': 600, 'reg_alpha': 0.4, 'reg_lambda': 6, 
    #'subsample': 0.8}
        ...
        #return preds
        #return self.average_predictions_by_doy(pd.Series(train_residual, index=X_test.index))
    ...
    def _ensemble_model(self, train, test):
        ...
        df.dropna(inplace=True)
#         features = ['Sales_lag1','Redemption_lag1','Redemption_lag7','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear', 
#                     'seasonal_base', 'lag7', 'lag14', 'lag28','lag30','lag56']
        ...
        feature_names = ['Redemption_lag1','Redemption_lag7','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear', 
                    'seasonal_base', 'lag7', 'lag14', 'lag28','lag30','lag56']
...
#         df['weekofyear'] = df.index.isocalendar().week.astype(int)
        ...
        features = ['Redemption_lag7','Redemption_lag1','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear', 'seasonal_base','lag3', 'lag7', 'lag28']
        X_train = df.loc[train.index.intersection(df.index), features]
        y_train = df.loc[train.index.intersection(df.index), self.target_col]
        X_test = df.loc[test.index.intersection(df.index), features]
        print(X_train.columns)
        ...
        ############################################
#         model_lgbm = lgb.LGBMRegressor(objective='regression', random_state=42,verbose=-1, n_jobs=-1)
...
#         # Grid search
#         grid_lgbm = GridSearchCV(model_lgbm, param_grid_lgbm, cv=2, scoring=scorer, verbose=40, n_jobs=-1)
...
#         model = lgb.LGBMRegressor(n_estimators=700,learning_rate=0.05,max_depth=5,
...
#         df['weekofyear'] = df.index.isocalendar().week.astype(int)
        ...
        df.dropna(inplace=True)
        #print(df)
        features = ['Redemption_lag7','Redemption_lag1','dayofweek', 'dayofyear', 'monthly', 'season','weekofyear','seasonal_base', 'lag7', 'lag28']
...
#         # Perform grid search to find the best set of hyper parameters
#         rf_grid_search = GridSearchCV(rf_classifier, rf_param_grid, cv=5, verbose=20,scoring='neg_mean_absolute_error',n_jobs=-1)
...
#         #############################
        ...
        index=model.feature_names_in_,
        columns=['feature importance'])
        feat_imp.sort_values('feature importance').plot(kind='barh', title='Feature Importance')
    ...
    def plot(self, preds, truth, label, conf_int=None):
        ...
        plt.title(f"{label} Forecast vs Actual Sales")
        plt.legend()
        plt.show()
